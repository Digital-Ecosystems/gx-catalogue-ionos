...
**WARNING** Make sure validations for `semantics`, `schema` and `signatures` are enabled in the `federated-catalogue` deployment.
...
Create `legalPerson.jsonld` file with the following content:
```json
{
